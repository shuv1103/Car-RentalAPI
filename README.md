...
# Waitlist Service — BullMQ + Redis Implementation
...
| `WaitListEntry` job | queued by `addToWaitlist()` | Log-only, no side effects — dashboard audit trail | payload `{ carId, userId, addedAt }`; handler just logs and returns `{ logged: true }` |
| `notifyNextUser` job | queued by `returnCar()` / by `bookingTimeout` handler | Pop next in line, open booking window | payload `{ carId }`; sets lock `EX 900`; schedules `bookingTimeout` at `delay: 900000` (15 min in ms) |
| `bookingTimeout` job | queued by `notifyNextUser` handler | Expire the window if unused | payload `{ carId, userId }`, `delay: 900000ms`; on fire, compares `GET lock` to `userId` |
| `workerInstance` | BullMQ `Worker("carWaitList")` | Concurrency + stall detection | `concurrency: 5`, `stalledInterval: 30000ms`, `maxStalledCount: 1`, `lockDuration: 30000ms` |
| Concurrent rent guard | `rental.service.js` → `rentCar` | Atomic decrement + lock check | `Car.findOneAndUpdate({carId, isAvailability:true, count:{$gt:0}}, {$inc:{count:-1}})`; non-lock-holder gets `ApiError(409, "...reserved for a waitlisted user...")` |
| Booking window constant | `rental.worker.js:8-9` | Single source of truth for timing | `BOOKING_WINDOW_SECONDS = 900`, `BOOKING_WINDOW_MINUTES = 15` |
